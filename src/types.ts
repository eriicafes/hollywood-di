...
type OmitKnownKey<T, U extends PropertyKey> = {
  [K in keyof T as Exclude<K, U>]: T[K];
};
export type Merge<T, To> = Pretty<
  RecordType<T> & OmitKnownKey<RecordType<To>, keyof T>
>;
...
export type HasIndexSignature<T> = KnownKey<keyof T> extends never
  ? true
  : false;
