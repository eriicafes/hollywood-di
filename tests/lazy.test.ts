...
describe("Lazy", () => {
    it("should eagerly initialize container", () => {
        const mockFn = vi.fn<[], "item">().mockImplementation(() => "item")
    ...
    it("should cascade container lazy option", () => {
        const mockFn1 = vi.fn<[], "item1">().mockImplementation(() => "item1")
        const mockFn2 = vi.fn<[], "item2">().mockImplementation(() => "item2")
    ...
    it("should delay circular dependency error", () => {
        expect(() => {
            Hollywood.create({
                circular1: factory((ctx: { circular2: string }) => "circular1" + ctx.circular2),
                circular2: factory((ctx: { circular1: string }) => "circular2" + ctx.circular1),
            })
        ...
        expect(() => circularContainer.resolve("circular1")).toThrowError("Circular dependency")
    })
})
